...
def schmitt_trigger(
    ...
    *,
    ...
    The output feeds back through ``R_upper / R_lower`` to create
    hysteresis.  Effective: ``V_hys ≈ V_supply × R_lower / (R_upper + R_lower)``.

    Returns ``(components, {'noninv_input'})``.  The ``noninv_input`` node
    carries both the input signal (via external connection) and the
...
def window_comparator(
    ...
    * Both LOW when signal is inside the window.
    ...
    Returns ``(components, {})``.  You need separate nodes for each output.
    """
    return [
